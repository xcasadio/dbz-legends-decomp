...
- `LoadCHBinFileAsync` charge le fichier disque dans un buffer CD.
...
- Les anciens bloqueurs rares de taille sont fermes eux aussi: `0x12 x_max_set` retourne `param_1 + 3`, `0x1B base_culY` retourne `param_1 + 7`, et `0x2C cheff_wait` retourne `param_1 + 1`. Le residu whole-stream vient donc surtout du branchement dependant de `g_animSharedVarTable`, pas d'une taille encore inconnue sur ces opcodes.
