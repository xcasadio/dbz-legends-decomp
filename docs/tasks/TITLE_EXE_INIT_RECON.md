...
Aucun code n'a ete ecrit pour cette etape.

Ce document a ete redige avant la fermeture du systeme de taches. Les symboles
qu'il cite portent desormais leurs noms fermes, appliques dans Ghidra et
journalises avec leur ancien nom dans `TITLE_EXE_TASK_SYSTEM.md`.
...
{
  __main();
  EnterCriticalSection();                        // syscall(0), EnterCriticalSection
  ResetCallback();
  ResetGraph(0);
  InitGeom();
  SetDispMask(0);
  ClearVram();                        // efface toute la VRAM
  PadInit(0);
  CdInit();
  do {
  ...
  srand(0x10000);
  ExitCriticalSection();                        // syscall(0), ExitCriticalSection
  FntLoad(0x3c0, 0x100);
  DAT_80083498 = FntOpen(0x10, 0x10, 0x100, 200, 0, 0x200);
  DAT_8008344c = 0;
  DAT_80083450 = 0;
  DAT_80083448 = 0;
  SetupGeometry(0xa8, 0x80, 0x1000, 0,0,0, 0x1000, 0,0,0);  // setup GTE
  CreateTask(FUN_80037388, 0,0,0,0, _DAT_80079854);       // enregistre une tache
  FUN_80037388();
...
|---|---|---|
| `0x80070B64` | `EnterCriticalSection` | `syscall(0)`, marquee *Possible A36.OBJ/EnterCriticalSection*. 9 appelants. |
| `0x80070E44` | `ExitCriticalSection` | `syscall(0)`, marquee *Possible A37.OBJ/ExitCriticalSection*. 10 appelants. |
| `0x80057508` | `ClearVram` | `ClearImage({0,0,0x400,0x200}, 0,0,0)` puis `DrawSync(0)`: efface la VRAM entiere en noir. 2 appelants. |
...
| `0x80057674` | `SetupGeometry` | Setup GTE: `SetGeomOffset`, `SetGeomScreen`, `SetFarColor(0x80,0x80,0x80)`, `SetBackColor(0x80,0x80,0x80)`, matrice couleur, `RotMatrix`/`SetLightMatrix`, `RotMatrix`/`SetRotMatrix`, puis ecriture directe de registres COP2 en `0x1F8000xx`. 3 appelants. |
...
`POLY_GT4_800B9518` et enregistrent `FUN_80038684` via `CreateTask`. Sa
...
|---|---|---|
| `0x80049504` | `CreateTask` | allocateur d'objets-taches, 42 appelants |
...
| `0x800587A8` | `RunFrameLoop` | appelee deux fois par tour de boucle |
...
- `RESOLVED`: la semantique de `CreateTask` est fermee, voir
