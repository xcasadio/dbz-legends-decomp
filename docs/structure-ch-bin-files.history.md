...
──────  ──────  ─────────────────────────────────────────────────────
+0x00   uint16  reloc_loop_bound  = nombre de dwords à relocaliser (6 observé sur CH_xx; 5/13/14/15/16/17/22/24/25/27 observés sur IN_xx)
...
+0x14   uint32  ptr_section_D     → pointeur compile-time (relocalisé) — usage PROBABLE : data anim raw
    [+0x18 pour les grands CH_BIN3 : slots relocalisés supplémentaires à partir de dw[6], en nombre variable selon reloc_loop_bound]
```
...
| Taille | 16–34 KB | 2–26 KB observés |
| `dword[0].low` (reloc_count) | 6 → relocalize dw[2..5] | variable : 5 sur `IN_IN/IN_OT2/IN_OUT`, 13..27 sur `IN_01..IN_10` |
| `dword[1]` (entry_count) | 37 pour CH_01 | 13 pour IN_01 |
| Header size | 24 bytes (6 dwords) | préfixe commun 24 bytes + slots relocalisés supplémentaires selon `reloc_loop_bound` |
| Pointeurs supplémentaires | 0 | présents sur les grands `IN_xx` quand `reloc_loop_bound > 6`; consommateur prouvé = `AnimCmd_ChEffSet` |
| Format entrées | CHBinMeshEntry 28-bytes | CHBinMeshEntry 28-bytes (13/13 fichiers `IN_xx` vérifiés sur disque) |
...
> **INCONNU** : le sens exact des slots supplémentaires de CH_BIN3. Leur existence variable et un consommateur réel (`AnimCmd_ChEffSet`) sont maintenant prouvés, et la structure des entrées est vérifiée sur les 13 fichiers `IN_xx`.
...
| `0x23` | `if_set` | `AnimCmd_IfSet (0x8003d450)` | CERTAIN | Conditionnel tagge + sauts forward `0x4xxx -> 0x8xxx` |
...
uint16 param_word:  3 specs de 5 bits dans `bits [4:0]`, `[9:5]`, `[14:10]`
                                        pour chaque spec:
                                            bits [3:0] = op_mode (0..14, `0xF` = skip)
                                            bit  [4]   = indirect_flag (0=valeur inline, 1=lecture via g_animSharedVarTable)
                                        le bit [15] n'est pas consommé par le parseur observé
uint16 operand_k:   une valeur par spec active (`spec.low4 != 0xF`)
```

**Exemple Entry0 stream `{0x1506, 0x2008, 0x0000, 0x0000, 0x0000}` :**
- object_id = 0x15 = 21 (index dans la table de transforms)
- spec0 = `0x08` -> op_mode=8, indirect=0, operand=0
- spec1 = `0x00` -> op_mode=0, indirect=0, operand=0
- spec2 = `0x08` -> op_mode=8, indirect=0, operand=0

**Note sur la progression du stream :**  
`AnimCmd_SetBodyPartTransforms` retourne `puVar6` qui a avancé de `2 + N` mots, avec `N = nombre de specs actives` (`spec.low4 != 0xF`). `ExecuteAnimStreamBatch` met à jour le pointeur courant et lit le prochain opcode.
...
| `0x23` | `0x8003d450` | `AnimCmd_IfSet` | `if_set` | CERTAIN |
...
#### Mode init (bit[15]=0) : return streamPtr+5
...
Correction ulterieure : la borne `+6` etait un off-by-one de parseur. La decompilation directe montre bien un retour `streamPtr + 5` pour la forme init.
...
### 27.7 AnimCmd_BitSet — `bit_set` (opcode 0x18 @ 0x8003b148, 280 octets)

**Format** : 3 mots

```
word[0]: opcode=0x18 | op_mode<<8
...
### 27.8 AnimCmd_BitChk — `bit_chk` (opcode 0x17 @ 0x8003afa4, 420 octets)

**Format** : 2-4 mots (selon mode)

```
word[0]: opcode=0x17 | flags<<8 | lo_byte
...
**Format minimal** : `1` ou `3` mots selon `bits[15:14]`

```
mode 00 (conditionnel, 3 mots)
word[0]: opcode=0x23 | tag12 (bits[11:0])
word[1]: hi_byte=varTable_src_idx, lo_byte flags:
         bit[4] = NOT/invert
         bit[5] = AND-all vs ANY
word[2]: bitmask a tester

mode 01/11 (scan-only, 1 mot)
word[0]: opcode=0x23 | 0x4000/0xC000 | tag12

mode 10 (marker, 1 mot)
word[0]: opcode=0x23 | 0x8000 | tag12
```
...
if ((word0 & 0xC000) == 0x0000) {
    val = g_animSharedVarTable[word1 >> 8]
    if (word1 & 0x10): val = ~val
    if (word1 & 0x20): fail = (val & word2) != word2
    else:              fail = (val & word2) == 0

    if (!fail):
        return param_1 + 3

    ptr = param_1 + 3
    while ((*ptr & 0x0FFF) != tag12): ptr++
    return ptr + 1
}

if ((word0 & 0x4000) != 0) {
    ptr = param_1 + 1
    while ((short)*ptr != (short)(0x8000 | tag12)): ptr++
    return ptr + 1
}

return param_1 + 1   // mode 10
```

**Usage observe** (CERTAIN sur le sous-corpus rebalaye) :

- Les `22` hits prefixes `low16=0` atteints dans `CH_BIN1/CH_BIN2` commencent tous par `0023 0000 4000`.
- Un bloc representatif (`CH_01.BIN E01`) montre la sequence `0023 0000 4000 ... 4023 0123 0000 2000 ... 4123 8123 8023`.
- Cela prouve en corpus que `4023/4123` servent de sauts forward mode `01`, et que `8023/8123` sont leurs marqueurs mode `10` correspondants.

**INCONNU** :

- Aucun usage distinct du mode `11` n'est encore observe dans le corpus courant, meme si le parseur Ghidra le fait suivre le meme chemin structurel que le mode `01`.
...
| 0x8003d450 | `AnimCmd_IfSet` | 0x23 | 1-3 mots + scan | Conditionnel tagge + sauts forward `0x4xxx -> 0x8xxx` |
...
### 28.6 AnimCmd_ChseCall — `chse_call` (opcode 0x2D @ 0x8003ec74, 228 octets)

**Format** : 2 mots

```
word[0]: opcode=0x2D | target_arg<<8 (ou varTable[lo_byte & 0xf] si bit[7])
...
- Dans `GAME.EXE`, aucun consommateur runtime direct de `unknown_0x08` n'a été observé à ce stade : `g_chBinEntryTableBasePtr` n'a que `3` XREF directs, tous internes à `RenderBattleScene3D` (`read/write/read` sur la même fonction), et ce champ n'y est pas lu.
- Dans `VS.EXE`, même constat : `g_chBinEntryTableBasePtr` n'a que `3` XREF directs, tous internes à `RenderBattleScene3D` (`read/write/read`), et `unknown_0x08` n'est pas lu non plus dans `AnimCmd_RenderEntryGroup`.
- `mesh.counts_packed = 0x00120001` n'apparaît qu'une seule fois dans le corpus actuel : `CH_31.BIN E4`.
- `mesh.counts_packed = 0x00010100` n'est pas un motif rare : `85` occurrences observées dans le corpus.
- `mesh.counts_packed = 0x00010100` traverse au moins `7` classes de `primitive_count_packed` observées (`0x00000000`, `0x00010001`, `0x00010002`, `0x00010003`, `0x00010004`, `0x00010005`, `0x00030003`).
- Pour les entrées `primitive_count_packed = 0x00010010`, la distribution de `mesh.counts_packed` est fortement dissymétrique : `0x00100001` -> `38` cas, `0x00010001` -> `2` cas seulement (`CH_26.BIN E12/E13`).
...
- Le `high16` correspond à `primitive_count_packed.high16` sur `790/792` entrées; seules `CH_09.BIN E6/E7` dévient (`primHi=3`, `u08Hi=1`).
...
| `u08.high16 == prim.high16` sur `790/792` entrées | CERTAIN | scan corpus complet; seules exceptions `CH_09 E6/E7` |
| `prim.high16 > 1` est un cas spécial très rare | CERTAIN | seulement `CH_09 E6/E7` et `CH_31 E4` |
| aucun lecteur runtime direct de `unknown_0x08` observé dans `GAME.EXE` | CERTAIN | `g_chBinEntryTableBasePtr` n'a que 3 XREF directs, tous dans `RenderBattleScene3D` |
| aucun lecteur runtime direct de `unknown_0x08` observé dans `VS.EXE` | CERTAIN | `g_chBinEntryTableBasePtr` n'a que 3 XREF directs, tous dans `RenderBattleScene3D`; `AnimCmd_RenderEntryGroup` saute aussi le slot `+0x08` |
...
| pour `primitive_count_packed = 0x00010010`, `mesh.counts_packed = 0x00010001` n'apparaît que sur `CH_26 E12/E13` | CERTAIN | scan corpus ciblé : `2` cas contre `38` cas en `0x00100001` |
| `mesh.counts_packed = 0x00120001` est unique dans le corpus | CERTAIN | seul `CH_31 E4` |
| `mesh.counts_packed = 0x00010100` ne suffit pas à expliquer `CH_09 E6/E7` | CERTAIN | `85` occurrences dans le corpus |
| `mesh.counts_packed = 0x00010100` n'est pas lié à une seule classe `prim` | CERTAIN | observé avec `prim = 0x00000000`, `0x00010001`, `0x00010002`, `0x00010003`, `0x00010004`, `0x00010005`, `0x00030003` |
...
| `CH_31 E4` | `0x00030012` | `0x00030001` | `0x00010004` | `0x00120001` | `0x00010001` | seul cas où `u08.high16=3` suit `primHi=3`; `mesh.counts = countX 0x12, countY 0x0001`; stream non nul |
| `CH_09 E6/E7` | `0x00030003` | `0x00010001` | `0x00010004` | `0x00010100` | `0x00010100` | anomalie: `primHi=3` mais `u08Hi=1`; `mesh/lighting.counts = countX 0x0001, countY 0x0100`; `E6` a un stream non nul, `E7` a `ptr_anim_stream = 0`, mais les listes vertex/mesh/light sont identiques |
...
- Les couples observes `(unknown_0x08.low16, primitive_count_packed.high16)` se limitent a `(0,0)`, `(1,1)`, `(1,3)`, `(2,1)`.
- `unknown_0x08.low16 = 0` est la seule classe observee avec `primitive_count_packed.high16 = 0`.
- `unknown_0x08.low16 = 0` couvre toutes les entrees `primitive_count_packed.low16 = 0`; inversement `unknown_0x08.low16 = 1` n'apparait jamais avec `primitive_count_packed.low16 = 0`.
- Sa forme dominante est `0x00010001`.
- Son `high16` recopie `primitive_count_packed.high16` sur `790/792` entrées; seules `CH_09 E6/E7` dévient.
- Dans `25` fichiers, les `3` premieres entrees sont `(entry_id_packed, primitive_count_packed, unknown_0x08) = (0,0,0)` repete trois fois, avec les memes pointeurs `vertex/mesh/light`; seuls `CH_06.BIN`, `IN_04.BIN`, `IN_08.BIN` mettent le troisieme `ptr_anim_stream` a `0`.
- Les `12` cas `unknown_0x08.low16 = 0` avec `primitive_count_packed.low16 = 1` portent `entry_id_packed.low16 = 0xXX00`; l'entree `low16 = 1` qui suit immediatement reste toujours dans la meme famille `0xXX**`, et `10/12` de ces sous-runs conservent ce tag jusqu'au prochain changement de classe.
...
- `CH_09 E6` et `CH_09 E7` partagent les mêmes listes vertex/mesh/light malgré une différence de `ptr_anim_stream` (`E6 != 0`, `E7 = 0`).
- `mesh.counts_packed = 0x00120001` est unique à `CH_31 E4` dans le corpus actuel.
- `mesh.counts_packed = 0x00010100` est courant et ne suffit donc pas, à lui seul, à expliquer l'anomalie `CH_09 E6/E7`.
- `mesh.counts_packed = 0x00010100` traverse plusieurs classes `prim`; il ne peut donc pas être lu comme une signature spécifique de `prim.high16 = 3`.
- Les deux anomalies `prim.high16 = 3` ne partagent pas la même forme d'itération 2D : `CH_31 E4` expose `countX=0x12, countY=1`, alors que `CH_09 E6/E7` exposent `countX=1, countY=0x100` sur les segments mesh/lighting.
- Dans la classe `primitive_count_packed = 0x00010010`, `CH_26 E12/E13` sont les seuls cas observés avec `mesh.counts_packed = 0x00010001`; tous les autres cas observés utilisent `0x00100001`.
- Aucun lecteur runtime direct de `unknown_0x08` n'a été confirmé dans `GAME.EXE` ni dans `VS.EXE` sur le chemin principal actuellement relié.
- `CH_26 E12` a une ouverture de stream observée unique dans le corpus (`batch vide`, puis `1207 0100`).

**PROBABLE**
- `unknown_0x08.low16` encode une classe/type d'entrée, où `0` marque une sous-famille d'entrees a zero primitive ou d'entetes locaux `prim=1`, `1` la classe de rendu ordinaire, et `2` la sous-classe rare corrélée a la forme compacte `CH_26 E12/E13`.
- Les anomalies `prim.high16 = 3` reflètent probablement deux sous-classes structurelles différentes plutôt qu'un seul cas spécial uniforme.
...
1. Examiner côté code les lecteurs de `mesh.counts_packed = 0x00120001`, car ce motif est désormais prouvé unique à `CH_31 E4`.
2. Comparer en détail les chemins lecteurs de `mesh.counts_packed = 0x00010001` et `0x00100001` dans la sous-classe `prim=0x00010010`, puisque cette séparation isole maintenant `CH_26 E12/E13`.
...
- `word6` vaut `0x0000` dans les vrais `load_set` recales sur un debut de commande `AnimStream`; les rares cas non nuls du scan heuristique se resolvent en faux positifs d'alignement.
...
| `word6` | nul sur les `load_set` valides recales; cas non nuls = faux positifs d'alignement | champ réservé probable | scan corpus CH_BIN + recadrage par batches/handlers | `CH_09 E0 +0x0304` = `bit_set 0518 0003 0040`; `CH_42 E0 +0x042C` = operande final de `trans_set 1206 2108 0003 0003 0003` |
...
- Les cas heuristiques `word6 != 0` se resolvent en faux positifs d'alignement plutôt qu'en variantes réelles de format.
...
- Dans `VS.EXE`, comme dans `GAME.EXE`, la table d'entrées CH_BIN relocalisée n'a que `3` XREF directs, tous internes à `RenderBattleScene3D`, et le champ `CHBinMeshEntry.unknown_0x08` n'y est pas lu.
...
### 48.7 Validation des faux positifs whole-stream sur les prefixes `low16=0`

#### 48.7.1 Résumé factuel

- L'extension du parseur exact avec `0x14 utylty` (2 mots, sauf mode `2` -> `3`), `0x17 bit_chk`, et la correction `0x0C eye_set` init -> `streamPtr + 5` ferme maintenant les premiers batches effectifs des `64` prefixes `low16=0` du sous-corpus disque sans opcode inconnu.
- Les deux hits whole-stream apparents sur `0x02 table_set` (`CH_19.BIN E1` @ mot `81`, `CH_19.BIN E2` @ mot `7`) disparaissent une fois le framing runtime respecté et `0x19 end_set` traité comme terminaison réelle.
- Après recadrage et fermeture supplémentaire de `0x2F voice_call`, un seul hit tardif reste reachable: `CH_14.BIN E0` atteint le batch `223: 040A 0001`, `0318 0000 0002`, `032D 4002`, `F92E 2040`.
- Dans `CH_14.BIN`, `pri_set group_id = 4` cible la famille d'entrée ordinaire `entry_id_packed.low16 = 0x0400`, représentée uniquement par `E6` (`primitive_count_packed = 0x00010001`, `ptr_anim_stream = 0`).
- La decompilation Ghidra actuelle de `AnimCmd_RenderEntryGroup` confirme toujours la taille fixe de `0x02`: le handler lit explicitement `uVar3 = streamPtr[1]` puis retourne `streamPtr + 2`.
- La decompilation Ghidra actuelle de `AnimCmd_VoiceCall` ferme le faux positif `CH_31.BIN E0`: quand `cmd.high8 & 0xC0 = 0x80`, le handler retourne `param_1 + 1`, pas `param_1 + 2`.
- Le batch tardif `CH_31.BIN E0` se relit donc en `802F` (`voice_call`, 1 mot dans ce mode), puis `022D 4002` (`chse_call`, 2 mots), puis terminator `0000`; ce n'est pas un `0x02 table_set`.

#### 48.7.2 Table des preuves

| Élément | Accès | Type minimal | Fonction | Preuve |
|--------|-------|--------------|----------|--------|
| `CH_19.BIN E2` mot `4 = 0x0019` | `end_set` reachable avant mot `7 = 0x0002` | terminaison de stream | `AnimCmd_EndSet` | batch valide `4:0019`, terminator `5`, `end_set` met `g_meshStreamPtrBuffer = 0` |
| `CH_19.BIN E1` mot `71 = 0x0019` puis mot `78 = 0x0019` | deux `end_set` reachables avant mot `81 = 0x0002` | terminaison de stream | `AnimCmd_EndSet` | relecture batch par batch cohérente avec le framing `terminator + 2` |
| `AnimCmd_BitSet` | `return param_1 + 3` | commande `0x18` fixe sur 3 mots | `AnimCmd_BitSet` | décompilation Ghidra `/GAME.EXE` `0x8003b148`: lecture de `param_1[1]`, `param_1[2]`, retour `param_1 + 3` |
| `CH_14.BIN E0` mot `223 = 0x040A` | début de batch reachable | `pri_set(group_id=4)` | `AnimCmd_AddPrimsToOT` | décomposition batch cohérente `040A 0001`, `0318 0000 0002`, `032D 4002`, `F92E 2040`, puis terminator `232` |
| `CH_14.BIN E6` `entry_id_packed.low16 = 0x0400` | unique entrée famille `group_id=4`, `ptr_anim_stream = 0` | cible de groupe statique | `RenderBattleScene3D` / `g_renderMetadataBuffer` | scan table d'entrées `CH_14.BIN`: seule entrée dont le byte haut de `low16` vaut `0x04` |
| `AnimCmd_RenderEntryGroup` | `uVar3 = streamPtr[1]`, `return streamPtr + 2` | commande `0x02` fixe sur 2 mots | `AnimCmd_RenderEntryGroup` | decompilation Ghidra `/GAME.EXE` `0x800374f4`: lectures `*streamPtr`, `streamPtr[1]`, retour `streamPtr + 2` |
| `AnimCmd_VoiceCall` | mode `cmd.high8 & 0xC0 = 0x80` | retourne `param_1 + 1` (1 mot) | `AnimCmd_VoiceCall` | decompilation Ghidra `/GAME.EXE` `0x8003eed8`: branche `uVar3 == 0x80`, appel éventuel `FUN_80065798`, retour `puVar4 = param_1 + 1` |
| `CH_31.BIN E0` mots `285..288 = 0x802F 0x022D 0x4002 0x0000` | batch tardif reachable | `voice_call(1 mot)` puis `chse_call(2 mots)` | `AnimCmd_VoiceCall`, `AnimCmd_ChseCall` | `0x802F` ne consomme pas `0x022D` comme opérande dans ce mode; `0x022D` devient donc la vraie commande suivante et `0x4002` son second mot |

#### 48.7.3 CERTAIN / PROBABLE / INCONNU

**CERTAIN**
- Les hits `0x02 table_set` de `CH_19.BIN E1/E2` sont des faux positifs dus à une exploration qui traversait du contenu post-`end_set`.
- `CH_14.BIN E0` atteint réellement un `0x0A pri_set` tardif sur un batch valide.
- `CH_31.BIN E0` n'atteint pas de `0x02 table_set` tardif: ce faux positif venait d'un parseur qui traitait `0x2F voice_call` comme un opcode fixe 2 mots.

**PROBABLE**
- Certains prefixes `low16=0` servent de têtes de contrôle capables de déclencher plus tard l'ajout OT d'un groupe ordinaire déjà overlayé, sans porter eux-mêmes de primitives propres.

**INCONNU**
- Existence d'autres cas tardifs comparables (`table_set`, `pri_set`, `eff_set`, etc.) après extension du parseur exact aux opcodes rares encore non fermés.

### 48.8 Fermeture de la famille transform comme goulot de parseur

#### 48.8.1 Résumé factuel

- Les décompilations Ghidra de `AnimCmd_SetBodyPartTransforms` (`0x06`), `AnimCmd_SetBodyPartTransforms_v2` (`0x07`) et `AnimCmd_SetBodyPartTransforms_v3` (`0x08`) montrent la même règle de taille: `streamPtr + 2 + N`, où `N` est le nombre de specs actives dans `word1`.
- `word1` n'est pas packé en `3 x 6 bits`; il est relu comme `3 x 5 bits` (`[4:0]`, `[9:5]`, `[14:10]`), avec `low4 = op_mode`, `bit4 = indirect`, `low4 == 0xF` => skip, et le bit `15` non consommé dans les parseurs observés.
- Le batch tardif `CH_14.BIN E0 @ 234` se relit maintenant proprement: `0318 0000 1000`, `1107 2108 0003 0003 0003`, `0018 0002 0800`, `0018 0003 0050`, `1106 3D0F 0003`, terminator `0000`.
- Une relecture whole-stream des `65` streams prefixes `low16=0` du sous-corpus `CH_BIN1/CH_BIN2`, avec les tailles exactes de `0x05/0x06/0x07/0x08/0x17/0x18/0x2D/0x2E/0x2F/0x30` et des handlers déjà fermés, déplace le bloqueur dominant vers `0x23 if_set` (`22` premiers hits).
- Les autres premiers bloqueurs plausibles encore réels deviennent rares: `0x1B` (`4` cas), `0x2C` (`2` cas), `0x12` (`1` cas). Les mots `0x40/0x80/0xC0/...` qui apparaissent ensuite sont hors plage d'opcode valide et reflètent seulement un désalignement aval.

#### 48.8.2 Table des preuves

| Élément | Accès | Type minimal | Fonction | Preuve |
|--------|-------|--------------|----------|--------|
| `0x06/0x07/0x08 word1` | 3 itérations, `op_mode = local_26 & 0xF`, `indirect = local_26 & 0x10`, puis `local_26 >>= 5` | `3 x 5-bit specs` | `AnimCmd_SetBodyPartTransforms*` | décompilations Ghidra `0x80038308`, `0x800384e0`, `0x800386a8` |
| `0x06/0x07/0x08` | `return puVarX` après avance conditionnelle de `puVarX = streamPtr + 2 + N` | taille exacte `2 + N` | `AnimCmd_SetBodyPartTransforms*` | même boucle de skip/consommation dans les trois handlers |
| `CH_14.BIN E0` batch `234` | `1107 2108 0003 0003 0003` | `rotate_set`, taille `5` | `AnimCmd_SetBodyPartTransforms_v2` | `word1=0x2108` -> specs `0x08,0x08,0x08`, donc `N=3` |
| `CH_14.BIN E0` batch `248` | `1106 3D0F 0003` | `trans_set`, taille `3` | `AnimCmd_SetBodyPartTransforms` | `word1=0x3D0F` -> specs `0x0F,0x08,0x0F`, donc `N=1` |
| scan sous-corpus `CH_BIN1/CH_BIN2` | premiers opcodes non fermés sur `65` streams prefixes `low16=0` | `0x23` dominant, `0x1B/0x2C/0x12` rares | parseur Python recale | comptage: `0x23=22`, `0x1B=4`, `0x2C=2`, `0x12=1` |

#### 48.8.3 CERTAIN / PROBABLE / INCONNU

**CERTAIN**
- La famille transform `0x06/0x07/0x08` n'est plus un goulot de parseur de taille.
- Le vieux modèle `3 x 6 bits` pour `0x06` est faux; le parseur observé consomme `3 x 5 bits` et n'utilise pas le bit `15`.
- Sur le sous-corpus rebalayé, `0x23 if_set` est maintenant le premier bloqueur majoritaire.

**PROBABLE**
- Après fermeture de `0x23 if_set`, les prochains vrais bloqueurs structurels devraient être la petite grappe `0x1B`, `0x2C`, `0x12` plutôt que la famille transform.

**INCONNU**
- Format de retour exact de `0x23 if_set` sur toutes ses variantes de scan.
- Taille exacte encore exploitable de `0x1B base_culY`, `0x2C cheff_wait`, `0x12 x_max_set` dans les cas whole-stream rencontrés.

### 48.9 Fermeture structurelle de `if_set` et des anciens bloqueurs rares `0x12/0x1B/0x2C`

#### 48.9.1 Résumé factuel

- La décompilation Ghidra de `AnimCmd_IfSet` montre trois formes structurelles distinctes: mode `00` conditionnel sur `g_animSharedVarTable`, modes `01/11` en scan forward `1` mot vers `0x8000 | tag12`, et mode `10` en marqueur `1` mot.
- Sur les `22` hits prefixes `low16=0` atteints dans `CH_BIN1/CH_BIN2`, le premier `if_set` est toujours `0023 0000 4000`; un dump représentatif (`CH_01.BIN E01`) montre ensuite les paires `4023 -> 8023` et `4123 -> 8123`.
- `AnimCmd_XMaxSet` (`0x12`) lit `word1`, `word2`, puis retourne `param_1 + 3`.
- `AnimCmd_BaseCulY` (`0x1B`) lit `word1`, `word2` et `4` opérandes supplémentaires, puis retourne `param_1 + 7`.
- `AnimCmd_CheffWait` (`0x2C`) ne lit pas d'opérande supplémentaire et retourne `param_1 + 1`.
- Le résidu whole-stream n'est donc plus un problème de taille brute sur `0x23/0x12/0x1B/0x2C`, mais de résolution de branchement dépendante de l'état runtime.

#### 48.9.2 Table des preuves

| Élément | Accès | Type minimal | Fonction | Preuve |
|--------|-------|--------------|----------|--------|
| `0x23` mode `00` | lit `word1`, `word2`, puis `return param_1 + 3` ou scan jusqu'au premier mot `(*ptr & 0x0FFF) == tag12` | commande `3` mots + branchement taggé | `AnimCmd_IfSet` | décompilation Ghidra `0x8003d450` |
| `0x23` modes `01/11` | lit seulement `word0`, puis scan jusqu'à la sentinelle signée `0x8000 | tag12` | saut forward `1` mot | `AnimCmd_IfSet` | même décompilation, branche `if ((word0 & 0x4000) != 0)` |
| `0x23` mode `10` | `return param_1 + 1` sans lecture d'opérande | marqueur `1` mot | `AnimCmd_IfSet` | même décompilation, chute hors des deux branches précédentes |
| `CH_01.BIN E01` | `0023 0000 4000 ... 4023 ... 4123 8123 8023` | bloc `if/skip/marker` imbriqué | scan disque recale | `4023/4123` mènent à `8023/8123` avec le même `tag12` |
| `22` hits prefixes `low16=0` | premier `if_set` toujours `0023 0000 4000` | forme d'ouverture observée | scan `CH_BIN1/CH_BIN2` | `22/22` occurrences atteintes |
...
- `control.bits2..3` ne sont jamais lus par le handler actuellement relié.
...
| `word0.high8.bits2..3` | aucune lecture prouvée | bits ignorés par le décodeur actuel | `AnimCmd_AsyncLoadTexture` | le handler ne masque que `0x80`, `0x03`, et `0x70` |
...
| `word2` init | `0x0000` sur tous les hits plausibles | champ réservé/zero-filled probable | scan corpus CH_BIN | 26/26 exemples plausibles observés à zéro |
...
- `bits2..3` ne sont pas consommés par le décodeur async actuellement relié.
...
## Section 49 — Plan d'action continu pour un agent IA

### 49.1 Objectif principal

- But unique prioritaire : converger vers la structure des fichiers `CH-x.BIN` et des variantes proches, avec preuve observable dans Ghidra, le corpus et, seulement si nécessaire, l'émulateur.
- Toute tâche doit être évaluée selon une seule question : est-ce qu'elle réduit une zone d'ombre structurelle du format ?
- Si la réponse est non, la tâche est reportée derrière toute tâche qui améliore directement la compréhension de la structure.

### 49.2 Règles d'exécution pour l'agent

1. L'agent travaille en continu tant qu'il existe une tâche non bloquée qui rapproche de la structure du fichier.
2. Si une nouvelle piste apparaît en cours d'analyse, l'agent crée immédiatement une nouvelle tâche dans cette section, avec priorité et statut, puis l'exécute sans s'arrêter si elle est plus importante que la tâche en cours.
3. L'agent met à jour ce fichier à chaque preuve solide : nouveaux lecteurs, nouvelles corrélations, nouveaux champs, nouvelles limites prouvées.
4. L'agent met à jour Ghidra en parallèle quand une équivalence, un nom ou un commentaire atteint le niveau CERTAIN.
5. L'agent n'utilise l'émulateur que si l'analyse statique est bloquée sur une ambiguïté sémantique restante.
6. L'agent évite les recherches globales par regex qui provoquent des timeouts ; il privilégie les lectures locales, les XREF ciblés, les call graphs locaux, les constantes et les cas rares déjà identifiés.
7. L'agent ne s'arrête que si toutes les tâches prioritaires sont terminées ou si le prochain pas exige une preuve inaccessible avec les outils disponibles.

### 49.3 Échelle de priorité

| Priorité | Règle |
|----------|-------|
| `P0` | tâche qui prouve directement un champ, un tableau, un pointeur, une taille, un format de commande, un framing, ou un lecteur/absence de lecteur d'un champ encore INCONNU |
| `P1` | tâche qui sépare deux hypothèses structurelles concurrentes ou qui ferme une anomalie rare du corpus |
| `P2` | tâche de consolidation : renommage, partage de types, nettoyage de commentaires, validation complémentaire |
| `P3` | tâche de confort documentaire ou de validation non nécessaire à la structure |

### 49.4 Tableau de tâches vivant

Statuts autorisés : `TODO`, `IN_PROGRESS`, `DONE`, `BLOCKED`

| ID | Priorité | Statut | Tâche | Critère de fin |
|----|----------|--------|-------|----------------|
| `T1` | `P0` | `DONE` | Identifier tout lecteur réel de `CHBinMeshEntry.unknown_0x08` dans `GAME.EXE` et `VS.EXE` | absence de lecteur direct prouvée sur les chemins CH_BIN reliés : `g_chBinEntryTableBasePtr` n'a que 3 XREF directs dans chaque EXE, tous internes à `RenderBattleScene3D`, et `AnimCmd_RenderEntryGroup` saute aussi `entry[2]` |
| `T2` | `P0` | `DONE` | Expliquer les cas rares `unknown_0x08.low16 = 2` (`CH_26 E12/E13`) par une différence de structure observable | propriété minimale désormais prouvée : dans la classe `prim=0x00010010`, seuls `CH_26 E12/E13` utilisent `mesh.counts_packed = 0x00010001`; les autres cas observés utilisent `0x00100001` |
| `T3` | `P0` | `IN_PROGRESS` | Expliquer les cas `primitive_count_packed.high16 = 3` (`CH_09 E6/E7`, `CH_31 E4`) | le rôle minimal du `high16` est borné par preuve ou l'anomalie est réduite à un sous-ensemble précis de lecteurs |
| `T4` | `P0` | `IN_PROGRESS` | Finir l'inventaire des lecteurs des champs de `CHBinMeshEntry` et des structures segmentées associées | chaque champ `+0x00..+0x18` a sa table de lecteurs/propriétés minimales |
| `T5` | `P1` | `TODO` | Distinguer structurellement `ptr_section_4` et `ptr_section_5` au-delà de la statistique de corpus | une différence de format, de framing ou de consommateur aval est prouvée |
| `T6` | `P1` | `IN_PROGRESS` | Cartographier les handlers `AnimStream` encore anonymes reliés aux cas rares du corpus | chaque handler ciblé reçoit un nom minimal et une lecture structurelle liée au format |
| `T7` | `P1` | `TODO` | Fermer la structure minimale des compteurs packés des segments vertex/mesh/lighting | le sens minimal de chaque halfword utile est prouvé ou borné |
| `T8` | `P2` | `TODO` | Vérifier que les structures CH_BIN partagées via `DbzLegendsTypes` restent la source de vérité pour `GAME.EXE` et `VS.EXE` | types partagés appliqués et cohérents avec les commentaires prouvés |
| `T9` | `P2` | `TODO` | Employer PCSX-Redux uniquement sur les ambiguïtés restantes après blocage statique | une question sémantique résiduelle est validée ou écartée sans introduire de nouvelle incertitude |

### 49.5 Politique d'ajout automatique de nouvelles tâches

- Si une nouvelle fonction, constante, structure, table, variante de stream ou cas rare est découvert et qu'il peut expliquer un champ INCONNU, créer immédiatement une tâche `P0` ou `P1` dans le tableau ci-dessus.
- Si une nouvelle piste n'a qu'un intérêt documentaire, créer au plus une tâche `P2`.
- Toute nouvelle tâche doit contenir : `ID`, `Priorité`, `Statut`, `Tâche`, `Critère de fin`.
- Si la nouvelle piste bloque la tâche en cours et est plus structurante, l'agent bascule dessus sans attendre.
- Quand une tâche est terminée, l'agent la marque `DONE` puis passe immédiatement à la tâche non bloquée de plus haute priorité.

### 49.6 Ordre de traitement obligatoire

1. Prouver les lecteurs et non-lecteurs des champs encore ambigus.
2. Résoudre les cas rares du corpus qui cassent les corrélations majoritaires.
3. Compléter les tables minimales des structures partielles.
4. Consolider les noms, types partagés et commentaires.
5. Ne faire de validation runtime que pour trancher une ambiguïté que la statique ne peut plus réduire.

### 49.7 Conditions de blocage légitime

Une tâche ne peut être marquée `BLOCKED` que si :

- aucun lecteur, XREF, call path local ou cas corpus adjacent n'est disponible ;
- l'étape suivante exige un état runtime stable indisponible ;
- ou bien la preuve nécessaire dépend d'un outil non exposé.

Dans ce cas, l'agent doit :

1. écrire explicitement pourquoi la tâche est bloquée ;
2. proposer le plus petit prochain pas permettant de la débloquer ;
3. passer immédiatement à la tâche non bloquée suivante la plus prioritaire.

### 49.8 Définition de réussite

Le plan n'est réussi que lorsque :

- la structure minimale des fichiers `CH-x.BIN` est prouvée champ par champ au niveau nécessaire ;
- les variantes rares ont une lecture minimale ou un bornage explicite ;
- les commandes `AnimStream` utiles au format sont reliées à leurs consommateurs ;
- et les zones restantes INCONNU sont clairement résiduelles, localisées et justifiées.

---

## Section 50 — Lecteurs des buffers dérivés de `CHBinMeshEntry`

### 50.1 Résumé factuel

- Les lecteurs runtime restants ne relisent pas directement `CHBinMeshEntry`; ils consomment des buffers dérivés initialisés par `RenderBattleScene3D` et `AnimCmd_RenderEntryGroup`.
- Dans `GAME.EXE` comme dans `VS.EXE`, les XREF de `g_chBinEntryTableBasePtr` restent bornées à `RenderBattleScene3D`; aucun autre lecteur brut actuellement relié dans les branches combat/effet ne relit directement la table `CHBinMeshEntry`.
- `g_renderMetadataBuffer` est relu CERTAINEMENT par `AnimCmd_SetMeshPaletteRange`, `AnimCmd_AddPrimsToOT` et `AnimCmd_ChEffSet` dans `GAME.EXE`; `VS.EXE` présente les mêmes homologues exacts, y compris `AnimCmd_ChEffSet` (`0x8003dcbc`).
- `entry_id_packed.high16` alimente CERTAINEMENT `g_meshEntryFlagsHiBuf`.
- `g_meshEntryFlagsHiBuf` est relu CERTAINEMENT par `AnimCmd_XAddSet` et `AnimCmd_XMaxSet` dans `GAME.EXE`; `VS.EXE` présente les mêmes XREF structuraux aux adresses homologues (`0x80039d80..0x8003a154`).
- Dans `GAME.EXE`, `AnimCmd_XMaxSet` parcourt CERTAINEMENT `g_meshEntryFlagsHiBuf` comme un tableau de `short` et applique un clamp à `0` sur les valeurs négatives après `ApplyMathOp`.
- Dans `GAME.EXE`, `AnimCmd_XAddSet` relit CERTAINEMENT `g_meshEntryFlagsHiBuf` dans la même boucle qui manipule `g_meshXOffsetBuffer`.
- `primitive_count_packed.low16` alimente CERTAINEMENT `g_meshCountBuffer`.
- Dans `GAME.EXE` comme dans `VS.EXE`, `RenderBattleScene3D` et `AnimCmd_RenderEntryGroup` n'écrivent dans `g_meshCountBuffer` que `(short)primitive_count_packed`; aucun buffer dérivé du chemin d'overlay actuellement relié ne reçoit un reflet prouvé de `primitive_count_packed.high16`.
- `g_meshCountBuffer` est relu CERTAINEMENT par plusieurs handlers d'animation et de couleur dans `GAME.EXE`: `AnimCmd_AnimateVertexColors`, `AnimCmd_AnimatePolyColorRGBA`, `AnimCmd_PartsLink`, `AnimCmd_Rgb2Set`, `AnimCmd_BaseCulX`, `AnimCmd_BaseCulY`, `AnimCmd_BaseCulZ`, `AnimCmd_BaseCulP`, `AnimCmd_Uv0123Set`, `AnimCmd_Xy0123Set`.
- `VS.EXE` présente les mêmes familles d'accès à `g_meshCountBuffer` sur les plages homologues (`0x800393b0/0x800394f4`, `0x80039a6c`, `0x80039eb0`, `0x8003a4ec/0x8003a608`, `0x8003b3e8/0x8003b50c`, `0x8003b870/0x8003b998`, `0x8003bd04/0x8003be2c`, `0x8003c198/0x8003c2c0`, `0x8003cbec/0x8003cd04`, `0x8003d734/0x8003d86c`, etc.).
- L'alignement tardif de la table de dispatch prouve aussi `0x8003a1ac = AnimCmd_Rgb2Set` et `0x8003a6c4 = AnimCmd_Utility` dans `VS.EXE`; le premier relit les buffers dérivés comme dans `GAME.EXE`, le second n'appelle côté `GAME.EXE` que des helpers corps/maths (`ResolveBodyPartTarget`, `ResolveBodyPartScale`, `ComputeCharMovement`, matrices/GTE) et ne montre toujours pas de canal visible vers `primitive_count_packed.high16`.
- La queue complète de `g_animStreamDispatchTable` est désormais alignée dans `VS.EXE` jusqu'à `AnimCmd_ObjIntGet`, `AnimCmd_ObjLongGet`, `AnimCmd_BitChk`, et `AnimCmd_CheffWait`; ces handlers tardifs n'introduisent pas non plus de lecture prouvée de `primitive_count_packed.high16`.
- La branche effet dédiée `SpawnEffectTask -> EffectTaskMainLoop` est maintenant bornée CERTAINEMENT: `EffectTaskMainLoop` exécute `ProcessEntityScript`, rend une liste de sprites via `RenderTransformedSprites`, puis détruit éventuellement la task; ce chemin ne relit pas `CHBinMeshEntry` et n'introduit pas non plus de reflet visible de `primitive_count_packed.high16`.
- Le corpus disque affine le rôle minimal de `mesh.counts_packed`: `0x00010100` n'est pas un motif rare isolé mais une famille large (`85` occurrences) répartie sur au moins `7` classes de `primitive_count_packed`, alors que `0x00120001` reste actuellement isolé à `CH_31.BIN E4`.
- Les seuls cas corpus où `primitive_count_packed.high16 > 1` restent `CH_09.BIN E6/E7` (`prim=0x00030003`, `m=0x00010100`) et `CH_31.BIN E4` (`prim=0x00030012`, `m=0x00120001`).
- Donc, les halfwords utiles de `CHBinMeshEntry` continuent d'être consommés indirectement via ces buffers dérivés, alors que `unknown_0x08` reste non lu dans les chemins actuellement reliés et que `primitive_count_packed.high16` n'a pas encore de reflet prouvé dans les buffers d'overlay.

### 50.2 Table des preuves

| Buffer dérivé | Source minimale | Lecteurs prouvés `GAME.EXE` | Lecteurs observés `VS.EXE` | Lecture minimale |
|---------------|-----------------|-----------------------------|----------------------------|------------------|
| `g_renderMetadataBuffer` | `entry_id_packed.low16` | `AnimCmd_SetMeshPaletteRange`, `AnimCmd_AddPrimsToOT`, `AnimCmd_PartsLink`, `AnimCmd_Rgb2Set`, `AnimCmd_ChEffSet` | mêmes familles d'accès homologues, dont `AnimCmd_ChEffSet` (`0x8003dcbc`) | identifiant / paramètre aval recopié par entrée |
| `g_meshEntryFlagsHiBuf` | `entry_id_packed.high16` | `AnimCmd_XAddSet`, `AnimCmd_XMaxSet` | mêmes XREF structuraux autour de `0x80039d80..0x8003a154` | état `short` par entrée consommé par les handlers X-* |
| `g_meshXOffsetBuffer` | état runtime par entrée, initialisé à `0` | `AnimCmd_XAddSet`, `AnimCmd_PartsLink`, `AnimCmd_ObjLongGet` | mêmes XREF structuraux autour de `0x80039d54..0x80039dc4`, `0x80039f7c`, `0x8003ae08` | décalage/runtime offset par mesh |
| `g_meshCountBuffer` | `primitive_count_packed.low16` | `AnimCmd_AnimateVertexColors`, `AnimCmd_AnimatePolyColorRGBA`, `AnimCmd_PartsLink`, `AnimCmd_Rgb2Set`, `AnimCmd_BaseCulX`, `AnimCmd_BaseCulY`, `AnimCmd_BaseCulZ`, `AnimCmd_BaseCulP`, `AnimCmd_Uv0123Set`, `AnimCmd_Xy0123Set` | mêmes familles d'accès homologues, dont `0x8003b184`, `0x8003b604`, `0x8003ba98`, `0x8003bf2c`, `0x8003c9ec`, `0x8003d42c` | nombre de primitives/éléments actifs pour les handlers aval |
| `g_meshStreamPtrBuffer` | `ptr_anim_stream + 4` | `ExecuteAnimStreamBatch`, `AnimCmd_BitChk`, `AnimCmd_EndSet` | `ExecuteAnimStreamBatch`, `AnimCmd_BitChk` (`0x8003ae50`), `AnimCmd_EndSet` (`0x8003b10c`) | pointeur courant du batch/stream par mesh |
| `g_meshOffsetBuffer` | `*(uint16 *)(ptr_anim_stream + 2)` | `ExecuteAnimStreamBatch`, `AnimCmd_BitChk`, `AnimCmd_EndSet` | `ExecuteAnimStreamBatch`, `AnimCmd_BitChk` (`0x8003ae50`), `AnimCmd_EndSet` (`0x8003b10c`) | compteur/flag d'avancement du stream par mesh |

Preuve d'overlay commune `GAME.EXE` / `VS.EXE` :
- `RenderBattleScene3D` finit chaque entrée par `*meshCountBuffer = (short)*local_38` / `*local_58 = (short)uVar8`, où `local_38` (`GAME`) et `local_98+1` (`VS`) pointent sur `primitive_count_packed`.
- `AnimCmd_RenderEntryGroup` fait de même via `*local_58 = (short)*puVar12` (`GAME`) et `*psStack_58 = (short)*puVar12` (`VS`).
- Aucun de ces quatre overlays ne montre une copie prouvée de `primitive_count_packed.high16` vers un buffer dérivé du pipeline actuellement relié.

### 50.3 CERTAIN / PROBABLE / INCONNU

**CERTAIN**
- Le chemin principal de consommation des métadonnées d'entrée passe par des buffers dérivés, pas par une relecture directe systématique de la table `CHBinMeshEntry`.
- Dans `GAME.EXE` comme dans `VS.EXE`, le seul lecteur brut actuellement prouvé de `g_chBinEntryTableBasePtr` reste `RenderBattleScene3D`.
- `g_meshEntryFlagsHiBuf` et `g_meshCountBuffer` ont des lecteurs runtime explicites dans `GAME.EXE`.
- `g_meshStreamPtrBuffer` et `g_meshOffsetBuffer` sont maintenant fermés CERTAINEMENT dans `GAME.EXE` comme dans `VS.EXE`: hors overlays/init et `ExecuteAnimStreamBatch`, leurs seuls lecteurs/écrivains prouvés restent `AnimCmd_BitChk` et `AnimCmd_EndSet`.
- `VS.EXE` présente les mêmes groupes d'accès structuraux sur les mêmes buffers dérivés.
- `primitive_count_packed.high16` n'est pas reflété de manière prouvée dans les buffers d'overlay actuellement reliés; seul le `low16` est recopié dans `g_meshCountBuffer`.
- La branche effet `SpawnEffectTask -> EffectTaskMainLoop` est négative pour une relecture brute de `CHBinMeshEntry`; elle consomme des scripts d'entité, des body-part pointers et des listes de sprites transformés.

### 50.3 bis Homologues `VS.EXE` déjà bornés par profils d'XREF

| Adresse `VS.EXE` | Homologue `GAME.EXE` | Statut | Preuve minimale |
|------------------|----------------------|--------|-----------------|
| `0x80038720` | `AnimCmd_SetMeshPaletteRange` | CERTAIN | décompilation concordante + même lecture de `g_renderMetadataBuffer` que l'homologue `GAME.EXE` |
| `0x80038a34` | `AnimCmd_AddPrimsToOT` | CERTAIN | décompilation concordante + même lecture de `g_renderMetadataBuffer` et même boucle d'ajout à l'ordering table |
| `0x8003913c` | `AnimCmd_AnimateVertexColors` | CERTAIN | entrée de dispatch `AnimStream` + même paire d'XREF `g_meshCountBuffer` qu'en `GAME.EXE`, avec offsets internes concordants |
| `0x80039c18` | `AnimCmd_XAddSet` | CERTAIN | entrée de dispatch `AnimStream` + mêmes XREF `g_meshXOffsetBuffer` et `g_meshEntryFlagsHiBuf`, avec mêmes offsets internes relatifs qu'en `GAME.EXE` |
| `0x80039df0` | `AnimCmd_PartsLink` | CERTAIN | entrée de dispatch `AnimStream` + mêmes lectures de `g_meshCountBuffer` et `g_meshXOffsetBuffer`, avec offsets internes concordants |
| `0x8003a034` | `AnimCmd_XMaxSet` | CERTAIN | entrée de dispatch `AnimStream` + même grappe d'XREF `g_meshEntryFlagsHiBuf`, avec offsets internes concordants |
| `0x80039600` | `AnimCmd_AnimatePolyColorRGBA` | CERTAIN | décompilation concordante + même lecture de `g_meshCountBuffer` et même forme de boucle couleur que l'homologue `GAME.EXE` |
| `0x8003a1ac` | `AnimCmd_Rgb2Set` | CERTAIN | alignement par index de dispatch + décompilation concordante, avec lectures homologues de `g_renderMetadataBuffer` et `g_meshCountBuffer` |
| `0x8003a6c4` | `AnimCmd_Utility` | CERTAIN | alignement par index de dispatch + décompilation concordante; côté `GAME.EXE`, ses callees sont bornés à des helpers corps/maths et ne révèlent pas de lecteur `primitive_count_packed.high16` |
| `0x8003abe4` | `AnimCmd_ObjIntGet` | CERTAIN | alignement par index de dispatch + décompilation concordante; calcule une distance entre deux body parts et l'écrit dans `g_animSharedVarTable` |
| `0x8003ad80` | `AnimCmd_ObjLongGet` | CERTAIN | alignement par index de dispatch + décompilation concordante; somme `g_meshXOffsetBuffer` sur une plage puis écrit dans `g_animSharedVarTable` |
| `0x8003ae50` | `AnimCmd_BitChk` | CERTAIN | alignement par index de dispatch + décompilation concordante; pilote des branches/rewind de stream via `g_meshOffsetBuffer` et `g_meshStreamPtrBuffer`, sans helper aval |
| `0x8003b10c` | `AnimCmd_EndSet` | CERTAIN | homologue exact de `GAME.EXE` `AnimCmd_EndSet`; termine ou rewinde le stream courant via `g_meshStreamPtrBuffer` et `g_meshOffsetBuffer`, sans lecture de `CHBinMeshEntry` |

**PROBABLE**
- `entry_id_packed.high16` sert de borne, clamp ou état X initial pour les handlers X-* via `g_meshEntryFlagsHiBuf`.
- `primitive_count_packed.low16` sert de cardinal d'itération aval pour plusieurs handlers couleur/liaison via `g_meshCountBuffer`.
- Une partie importante des handlers `AnimStream` encore anonymes de `VS.EXE` peut être nommée localement à partir des seuls profils d'XREF sur les buffers dérivés.

**INCONNU**
- Rôle exact des `high16` rares des compteurs packés quand les buffers dérivés sont relus par ces handlers.
- Existence d'un consommateur spécialisé de `primitive_count_packed.high16` hors `RenderBattleScene3D` / `AnimCmd_RenderEntryGroup` et hors branches déjà bornées (`AnimCmd_Utility`, `SpawnEffectTask -> EffectTaskMainLoop`, handlers effet homologues `EffSet` / `AttSet` / `ChDanSet` / `ChEffSet`).
- Possibilité que `primitive_count_packed.high16` soit inutilisé/réservé dans le runtime combat actuellement relié: INCONNU (preuve insuffisante).

### 50.4 Prochaines actions Ghidra recommandées

1. Chercher un site secondaire de propagation des compteurs packés à l'intérieur de `RenderBattleScene3D`, `AnimCmd_RenderEntryGroup`, `ExecuteAnimStreamBatch` et du rechargement des batches de stream, plutôt qu'un nouveau lecteur brut de `CHBinMeshEntry`.
2. Continuer l'inventaire champ->buffer->lecteur jusqu'à fermeture complète de `CHBinMeshEntry`; si aucun reflet supplémentaire de `primitive_count_packed.high16` n'apparaît, conserver provisoirement ce halfword en `INCONNU (preuve insuffisante)` avec hypothèse bornée “réservé/non consommé”.
3. Sur le corpus disque, prioriser les trois entrées où `primitive_count_packed.high16 > 1` (`CH_09.BIN E6/E7`, `CH_31.BIN E4`) et vérifier si la structure du stream ou des segments mesh/light révèle une propagation secondaire absente des cas ordinaires.

## Section 51 — Header relocalisable variable et contre-exemples metadonnees/geometrie

### 51.1 Résumé factuel

- Le header CH_BIN n'est pas un bloc fixe de `7` dwords valides: `RenderBattleScene3D` ne relocalise que les dwords `2 .. reloc_loop_bound-1`.
- Donc `+0x10` n'est un vrai slot runtime que si `reloc_loop_bound > 4`, et `+0x14` que si `reloc_loop_bound > 5`.
- Les contre-exemples minimaux sont maintenant bornés: `CH_39.BIN` et `CH_44.BIN` ont `reloc_loop_bound = 4`, avec `+0x10 = 0x03020100` non relocalisé; `CH_20.BIN` et `CH_24.BIN` ont `reloc_loop_bound = 5`, donc `+0x14` n'y est pas un slot runtime valide.
- Des slots étendus au-delà de `+0x14` sont réellement consommés dans `CH_BIN1/CH_BIN2`: `load_set tbl=6/7` dans `CH_04.BIN`, `load_set tbl=6/8/9` dans `CH_49.BIN`.
- Dans un même fichier, un triplet exact `(ptr_vertex_segment_list, ptr_mesh_segment_list, ptr_lighting_segment_list)` peut réapparaître avec plusieurs couples `(primitive_count_packed, unknown_0x08)`.
- La séparation minimale de `primitive_count_packed.high16` est maintenant bornée par corpus: la classe `0` n'apparaît qu'avec `low16 = 0` ou `1`, alors que la classe `1` n'apparaît jamais avec `low16 = 0`.
- Exemples certains: `CH_04.BIN` partage la même géométrie entre `E00 (0x00000000, 0x00000000)`, `E06 (0x00000001, 0x00000000)` et `E0C (0x00010002, 0x00010001)`; `CH_05.BIN` partage la même géométrie entre `E0B/E0C/E0D/E0E` avec `prim = 0x00010001/04/02/01` et `u08 = 0x00010001` constant.
- Cela borne plus fortement `primitive_count_packed` et `unknown_0x08` comme métadonnées d'entrée superposées à une géométrie réutilisée, pas comme des sélecteurs uniques de layout.

### 51.2 Table des preuves

| Élément | Accès | Type minimal | Fonction / corpus | Preuve |
|--------|-------|--------------|-------------------|--------|
| `reloc_loop_bound` | boucle `for i = 2 .. reloc_loop_bound-1` | borne des slots header runtime valides | `RenderBattleScene3D` (`GAME.EXE`, `VS.EXE`) | décompilations homologues, ajout `0x2E800` seulement sur ces dwords |
| `CH_39.BIN`, `CH_44.BIN` | `reloc_loop_bound = 4`, `+0x10 = 0x03020100`, `+0x14 = 0` | seul `ptr_entry_table` et `slot3` sont valides | dump brut disque | `+0x10` n'est pas relocalisé donc ne peut pas être un pointeur runtime |
| `CH_20.BIN`, `CH_24.BIN` | `reloc_loop_bound = 5`, `+0x14 = 0` | `slot4` valide, `slot5` absent | dump brut disque | seuls les dwords `2..4` sont relocalisés |
| `CH_04.BIN` | `reloc_loop_bound = 11`, `dword[6]=0x801A4FC0`, `dword[7]=0x801A4FE0` | slots étendus runtime valides | dump header + `load_set` | hits atteints `tbl=6`, `tbl=7` |
| `CH_49.BIN` | `reloc_loop_bound = 10`, `dword[6..9]` pointeurs `0x801A89DC/95E8/930C/932C` | slots étendus runtime valides | dump header + `load_set` | hits atteints `tbl=6`, `tbl=8`, `tbl=9` |
| `CH_04.BIN` géométrie partagée | même `(vp,mp,lp)` pour `E00`, `E06`, `E0C` | même base de segments, métadonnées d'entrée différentes | scan corpus intra-fichier | `prim/u08 = (0,0)`, `(1,0)`, `(0x00010002,0x00010001)` |
| `CH_05.BIN` géométrie partagée | même `(vp,mp,lp)` pour `E0B/E0C/E0D/E0E` | même base de segments, `prim.low16` variable | scan corpus intra-fichier | `prim = 0x00010001/04/02/01`, `u08` constant |
| `primitive_count_packed.high16` | distribution `0 -> low16 {0,1}`, `1 -> low16 {1,2,3,4,5,6,8,11,16,32,36,60,64,112,128}`, `3 -> low16 {3,18}` | classe d'entrée minimale, pas compteur de segments | scan corpus complet | aucune entrée `high16=1` n'a `low16=0`; aucune entrée `high16=0` n'a `low16 > 1` |

### 51.3 Structure partielle

```c
typedef struct CHBinFileHeaderPrefix {
    uint16_t reloc_loop_bound;  // +0x00 CERTAIN
    uint16_t header_flags;      // +0x02 CERTAIN
    uint32_t entry_count;       // +0x04 CERTAIN
    uint32_t ptr_entry_table;   // +0x08 CERTAIN
    uint32_t reloc_slot_3;      // +0x0C si reloc_loop_bound > 3
    uint32_t reloc_slot_4;      // +0x10 si reloc_loop_bound > 4
    uint32_t reloc_slot_5;      // +0x14 si reloc_loop_bound > 5
    // ... slots additionnels possibles tant que l'indice dword < reloc_loop_bound
} CHBinFileHeaderPrefix;

typedef struct CHBinMeshEntry {
    uint32_t entry_id_packed;        // +0x00 CERTAIN
    uint32_t primitive_count_packed; // +0x04 CERTAIN, mais pas un selecteur unique de layout
    uint32_t unknown_0x08;           // +0x08 CERTAIN, mais pas un selecteur unique de layout
    uint32_t ptr_vertex_segment_list;
    uint32_t ptr_mesh_segment_list;
    uint32_t ptr_lighting_segment_list;
    uint32_t ptr_anim_stream;
} CHBinMeshEntry;
```

### 51.4 CERTAIN / PROBABLE / INCONNU

**CERTAIN**
- `+0x10` et `+0x14` ne sont pas des pointeurs header universels; leur validité dépend de `reloc_loop_bound`.
- Le header runtime valide est un préfixe à longueur variable, pas un bloc fixe arrêté à `ptr_section_5`.
- Des slots étendus au-delà de `5` sont réellement consommés dans `CH_BIN1/CH_BIN2`.
- `primitive_count_packed` et `unknown_0x08` peuvent varier sans changement du triplet `(vertex,mesh,light)` dans un même fichier.

**PROBABLE**
- `primitive_count_packed.high16` et `unknown_0x08` décrivent une classe/rôle d'entrée superposée à une géométrie réutilisée, plutôt qu'un nouveau format de segment.

**INCONNU**
- Rôle sémantique exact de `primitive_count_packed.high16`.
- Rôle sémantique exact de `unknown_0x08`.
- Critère exact de répartition du contenu entre les slots texture bas (`3/4/5`) et leurs éventuels slots étendus.

### 51.5 Prochaines actions Ghidra recommandées

1. Renommer conceptuellement le "header commun" en préfixe relocalisable variable dans les notes et overlays futurs.
2. Relever, fichier par fichier, les premiers cas atteints de `tbl >= 6` pour distinguer les slots étendus vivants des simples dwords relocalisés non consommés.
3. Continuer les comparaisons intra-fichier "même géométrie, métadonnées différentes" pour borner davantage `primitive_count_packed.high16` et `unknown_0x08`.

## Section 52 — Gabarits statiques restants des branches `if_set`

### 52.1 Résumé factuel

- Les tailles importantes du bytecode `AnimStream` sont maintenant bornées; ce qui manque pour les branches est surtout l'état runtime de `g_animSharedVarTable`.
- Sur les `22` streams prefixes `low16=0` atteints dans `CH_BIN1/CH_BIN2` qui ouvrent par `if_set`, le premier test est toujours `if00(tag=0x023, w1=0x0000, mask=0x4000)`.
- La plupart de ces streams enchaînent ensuite un second test imbriqué `if00(tag=0x123, w1=0x0000, mask=0x2000)` avec les mêmes marqueurs `0x4xxx -> 0x8xxx` déjà prouvés.
- Les variantes tardives encore vues dans ce sous-corpus se limitent ensuite à des tests supplémentaires sur `0x1000`, `0x0800` ou `0x0400`, toujours sur `g_animSharedVarTable[0]`.
- Les écritures associées dans ces mêmes gabarits passent surtout par `bit_set` sur `var 0` avec les arguments `0x9FFF`, `0x2000`, `0x4000`, et sur `var 4` avec les arguments `0x0002`, `0x0003`, `0x0008`, `0x0010`, `0x0020` selon le fichier.
- Aucun save state in-battle exploitable n'est actuellement disponible dans PCSX-Redux pour trancher expérimentalement quelle branche est prise sur ces gabarits.

### 52.2 Table des preuves

| Élément | Accès | Type minimal | Source | Preuve |
|--------|-------|--------------|--------|--------|
| `22` streams prefixes `low16=0` | ouverture `0023 0000 4000` | test initial uniforme sur `varTable[0] & 0x4000` | scan corpus recale | `22/22` hits atteints |
| gabarit majoritaire | `if00(023,4000)` puis `if00(123,2000)` | squelette imbriqué de branche | scan corpus recale | `9` + `6` + `4` occurrences sur les patrons dominants |
| variantes tardives | `mask = 0x1000`, `0x0800`, `0x0400` | raffinements locaux | scan corpus recale | exemples `CH_01/05/06/07/10` |
| écritures associées | `bit_set` sur `dst=0` / `dst=4` | préparation de bits runtime | scan corpus recale | arguments observés dans les patrons détaillés |
| runtime actuel PCSX | header `0x801D2000 = 0`, streams nuls dans slots `1/2` de savestate | pas d'état de combat exploitable | lecture mémoire / savestates | `savestate` slots disponibles mais hors combat |

### 52.3 CERTAIN / PROBABLE / INCONNU

**CERTAIN**
- Le problème restant des branches `if_set` n'est plus un problème de taille de commande.
- Les gabarits statiques restants dans le sous-corpus prefixe `low16=0` sont concentrés sur `g_animSharedVarTable[0]` et quelques masques précis.

**PROBABLE**
- Une preuve runtime utile passera par un save state de combat où `g_meshStreamPtrBuffer` est non nul et où l'un de ces streams prefixes est actif.

**INCONNU**
- Valeurs runtime exactes de `g_animSharedVarTable[0]` sur les combats qui activent ces streams.
- Branche effectivement prise pour chaque gabarit observé.
