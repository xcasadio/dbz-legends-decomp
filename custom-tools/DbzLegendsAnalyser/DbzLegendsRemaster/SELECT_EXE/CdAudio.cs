...
//
// THE SELECT SCREEN'S MUSIC IS A DISC TRACK, NOT A SEQUENCE. FUN_80030908 @ 0x80030908 starts it
...
//
// This file carries FUN_80025788 @ 0x80025788 and every global it reads or writes.
internal static class CdAudio
{
    // GHIDRA: DAT_80055aa8 @ 0x80055AA8
    // .sbss, undefined4. The last CdSync status. FUN_80025658 zeroes it; FUN_80025788 stores
    // CdSync(1, NULL) into it every frame.
    // PARTIAL: no reader is on this slice's path, so what consumes the status is not closed.
    internal static int DAT_80055aa8;

    // GHIDRA: DAT_80055aac @ 0x80055AAC
    // .sbss, undefined4. The last CdReady result. Written and immediately tested by FUN_80025788;
    // the whole track-advance branch hangs off `== 1` (CdlDataReady).
    internal static int DAT_80055aac;
    ...
    // GHIDRA: DAT_80055abc @ 0x80055ABC
...
#pragma warning disable CS0649
    internal static int DAT_80055abc;
#pragma warning restore CS0649
    ...
    // GHIDRA: DAT_80055ad4 @ 0x80055AD4
    ...
    // DAT_80055adc, the four-byte CdlATV FUN_80025658 hands to CdMix. Eight bytes, which is what a
    // libcd result block is.
    internal static readonly byte[] DAT_80055ad4 = new byte[8];
    ...
    // GHIDRA: DAT_80055cec @ 0x80055CEC
    // .bss. THE DISC TOC, as CdlLOC entries. FUN_80025658 @ 0x80025658 closes both the element type
    // and the stride: it does `p = (CdlLOC *)&DAT_80055cec`, fills it with
    // `CdGetToc((CdlLOC *)&DAT_80055cec)` and then normalises entry by entry with `p = p + 1`. The
    // byte arithmetic every other reader uses agrees — FUN_80025788 below indexes
    // `&DAT_80055cec + DAT_80055abc * 4` and FUN_800258f0 `&DAT_80055cec + (track << 16 >> 14)`,
    // and 4 is sizeof(CdlLOC).
    // EXTENT: THIRTY-TWO entries, 128 bytes, 0x80055CEC..0x80055D6B. Closed by the neighbour above
    // it: 0x80055D6C is the first of the two 0x22-byte BIOS pad buffers FUN_800261a4 @ 0x800261A4
    ...
    internal static readonly CdlLOC[] DAT_80055cec = NewCdlLocArray(32);
    ...
    // GHIDRA: FUN_80025788 @ 0x80025788
    // 268 bytes, 7 call sites — the frame step FUN_800344a4 @ 0x800344A4 once per frame, plus the
    ...
    // module asked for (DAT_80055abc), it pauses or re-issues CdlPlay at the TOC entry — i.e. it is
    ...
    // LibCd.CdReady is a "Do nothing" stub returning 0, so DAT_80055aac is 0, the `== 1` test fails
    ...
    internal static void FUN_80025788()
    {
        int iVar1;
        ...
        if ((DAT_80055ac0 & 1) == 0)
        {
            DAT_80055aac = CdReady(1, DAT_80055ad4);
            ...
            if (DAT_80055aac == 1)
            {
                ...
                // on result byte 1.
                DAT_80055ab8 = (DAT_80055ad4[1] >> 4) * 10 + (DAT_80055ad4[1] & 0xf);
                if (DAT_80055abc < DAT_80055ab8)
                {
                    if ((DAT_80055ac0 & 4) != 0)
                    {
                        DAT_80055ae0 = 8;
                    ...
                    if ((DAT_80055ac0 & 2) != 0)
                    {
                        do
                        {
                            ...
                            // `&DAT_80055cec + DAT_80055abc * 4` on a char *, i.e. &toc[track].
                            iVar1 = CdControl(3, DAT_80055cec[DAT_80055abc], auStack_18);
                        } while (iVar1 == 0);
                    }
                }
            }

            DAT_80055aa8 = CdSync(1, null);
        }
    }
}
